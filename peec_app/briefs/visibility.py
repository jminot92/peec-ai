...
def _entity_order(summary_table: pd.DataFrame, owned_label: str) -> list[str]:
...
def _entity_colours(entities: list[str], owned_label: str) -> dict[str, str]:
...
def _build_competitive_rows(df: pd.DataFrame, owned_label: str) -> pd.DataFrame:
...
def competitor_options(df: pd.DataFrame, owned_label: str) -> list[str]:
    competitive_rows = _build_competitive_rows(df, owned_label)
    if competitive_rows.empty:
        return []
    competitor_totals = (
...
def build_visibility_chart_png(
    ...
    *,
    ...
    entity_order = _entity_order(summary_table, owned_label)
    colours = _entity_colours(entity_order, owned_label)
    ...
    legend = axis.legend(
        ...
        ncol=min(4, max(len(entity_order), 1)),
        frameon=False,
        fontsize=10,
    )
